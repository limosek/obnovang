...
COMMANDS += grep cut awk rsync reset openvt chvt wc ethtool dhclient3 bash mii-tool lsmod hostname ip rm chmod mv logger host zabbix_sender ssh killall ps pstree ls parted top locale find mc ssh-add ssh-agent mkfs mkfs.ext3 mkfs.vfat strace php5 ldd iftop chntpw
